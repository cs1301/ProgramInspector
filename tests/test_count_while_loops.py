...
def basic_tests():
    ...
    assert _test_file.count_while_loops(_test_file.globals.optimized_case) == 1
    assert _test_file.count_while_loops(_test_file.globals.optimized_case, ignore_optimized=True) == 0
...
def file_test():
    assert _test_file.count_while_loops() == 11
